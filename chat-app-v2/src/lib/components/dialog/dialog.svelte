...
<div id="backdrop" class="absolute top-0 left-0 w-full h-full bg-black select-none bg-opacity-20 z-20 cursor-default" on:click={() => show = false} transition:fade={{duration: 100, easing: cubicInOut}}>
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-5 min-w-64 min-h-32 bg-white rounded-3xl flex flex-col items-center justify-between gap-2 shadow-lg" on:submit={() => { show = false; submitAction(); } } on:click|stopPropagation transition:scale={{duration: 100, easing: cubicInOut}}>
